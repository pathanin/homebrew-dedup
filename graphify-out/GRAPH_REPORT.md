...
## Corpus Check
- 10 files · ~17,435 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 280 nodes · 397 edges · 23 communities (14 shown, 9 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 8 edges (avg confidence: 0.89)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0d5bbcc3`
...
1. `trash_files()` - 16 edges
2. `move_to_trash_safely()` - 15 edges
3. `_ensure_no_symlink_replacement()` - 13 edges
4. `trash_empty_dirs()` - 12 edges
5. `BrowserSelectionHandler` - 11 edges
6. `find_duplicates()` - 10 edges
7. `dedup` - 10 edges
8. `move_to_nas_recycle()` - 9 edges
9. `_decide_no_trash_strategy()` - 9 edges
10. `render_thumbnail()` - 8 edges
...
Cohesion: 0.07
Nodes (33): build_expected_hashes(), _decide_no_trash_strategy(), _deduplicate_by_ancestry(), _ensure_no_symlink_replacement(), load_send_to_trash(), move_to_local_trash(), move_to_trash_with_cmd(), Raised when a validated path is replaced with a symlink before removal. (+25 more)
...
Cohesion: 0.10
Nodes (25): BrowserSelectionHandler, BrowserSelectionState, build_browser_payload, build_thumbnail_command, describe_original_reason, DuplicateGroup, EmptyDirSelectionState, get_exif_info (+17 more)
...
Cohesion: 0.08
Nodes (23): Exception, EmptyDirSelectionState, find_nas_recycle_root(), get_macos_volume_root(), is_macos_external_volume_path(), move_to_trash_safely(), Raised when send2trash, /usr/bin/trash, and on-volume NAS recycle     folders ar, Return /Volumes/<name> for a /Volumes/<name>/... path, else None. (+15 more)
...
Cohesion: 0.20
Nodes (11): make_unique_collision_path(), move_to_nas_recycle(), Return ``path`` or a non-existing sibling with the process collision suffix., Return ``path`` or a non-existing sibling with the process collision suffix., Return ``path`` or a non-existing sibling with the process collision suffix., Return ``path`` or a non-existing sibling with the process collision suffix., Return ``path`` or a non-existing sibling with the process collision suffix., Move ``path`` into a NAS server-side recycle folder via os.rename.      Same-vol (+3 more)

### Community 22 - "Community 22"
Cohesion: 0.33
Nodes (6): prompt_permanent_delete(), Ask whether to permanently delete items on a volume with no trash.      Returns, Ask whether to permanently delete items on a volume with no trash.      Returns, Ask whether to permanently delete items on a volume with no trash.      Returns, Ask whether to permanently delete items on a volume with no trash.      Returns, Ask whether to permanently delete items on a volume with no trash.      Returns
...
- **Why does `trash_files()` connect `Trash & Volume Management` to `HTTP Server Infrastructure`, `Application Core & HTML Builders`, `Empty Directory Cleanup`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `BrowserSelectionHandler` connect `Media Metadata & Thumbnails` to `Community 8`?**
